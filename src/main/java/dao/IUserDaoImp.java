...
public class IUserDaoImp implements IUserDao {
        ...
        try {
            while (r.next()) {
                u = new User(
                        r.getString(1),
                        r.getString(2),
                        Integer.parseInt(Optional.ofNullable(r.getString(3)).orElse("0")),
                        Integer.parseInt(Optional.ofNullable(r.getString(4)).orElse("0")),
                        Integer.parseInt(Optional.ofNullable(r.getString(5)).orElse("0")));
                return u;
            }
    ...
    public List<User> findAll() {
        ...
        try {
            while (rs.next()) {
                ui = new User(rs.getString(1), rs.getString(2), Integer.parseInt(rs.getString(3)),
    ...
    public int findWinNum(String userName) {
        ...
        try {
            while (rs.next()) {
                u = new User(rs.getString("name"), rs.getString("fileName"),
                        Integer.parseInt(rs.getString("winNum")), Integer.parseInt(rs.getString("loseNum")),
                        Integer.parseInt(rs.getString("tiedNum")));
                t = u.getWinNum();
            }
        } catch (SQLException e) {
            // TODO Auto-generated catch block
